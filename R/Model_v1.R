...
case_data_setup <- outbreak_setup(num.initial.cases = 3,
                             ...
                             quarantine = NULL) 
{
  ...
  case.id.assignement<-function(infector.i){
    ...
    if(any(study_participants==infector.i)){
      ...
      # Number of expected contacts based on age group(take a study id and their contacts)
      nHH.con.infector=sample(study_participants_demo$id[age.similar],size = 1)
      # id of contacts
  ...
  prob_samples[,`:=`(type,ifelse(case.link==infector.link,"HH","non-HH"))]
  ...
  # --- change missed status for HH contacts to FALSE. Assuming all HH contacts
  # can be traced
  prob_samples$missed[prob_samples$type=='HH']<-FALSE
  ...
  prob_samples[, `:=`(c("incubfn_sample", "infector_iso_time", 
                        "infector_asym","type","case.link","infector.link"), NULL)]
...
outbreak_data_ext<-outbreak_model_ext(num.initial.cases = 5,prop.ascertain = 0.5,
...
outbreak_data<-outbreak_model(num.initial.cases = 5,prop.ascertain = 0.5,
                                  cap_max_days = 365,cap_cases = 2000,r0isolated = 0.5,
                                  r0community = 2.5,disp.iso = 1,disp.com = 0.16,
                                  k = 0.7,delay_shape = 2.5,delay_scale = 5,
                                  prop.asym = 0.3,quarantine = TRUE)
...
# Our extension
time.ext<-system.time(res.ext <- scenario_sim_ext(n.sim = 10,num.initial.cases = 5,cap_max_days = 365,
...
# Weekly cases
matplot(weekly.cases.lshtm,type='l',col='grey',lty=1,lwd=2)
