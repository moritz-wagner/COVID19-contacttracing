...
# prop.ascertain <- 0
# 
# case_data <- outbreak_setup_ext(num.initial.cases = num.initial.cases,case_ids = sample(CD[[1]]$csid,1),
                             ...
                             contact_data=NULL) 
{
  ...
  new_case_data2[,`:=`(con=min(length(unlist(contact_id)))),by=infector]
  ...
  ##Infections that didn't occur due to lack of contacts
  con.dep <- new_case_data2[,.(diff=max(N-con,0)),by=.(infector,N)]
  dep.con <- sum(con.dep$diff) ##Get the number of contact depletion to keep track of
  ...
  ##Remove any duplicates (worth couting these?)
...
#   nhh.inf.ages <- nhh.study_participants[csid%in%z[grepl("nhh",inf_id),inf_id],]
#   new.nhh.ids <- nhh.inf.ages[study_participants.age,nomatch=0L,on="age"][,.(new.csid=sample(unlist(i.csid),1)),by=csid]
#   nhh.study_participants[csid%in%z[grepl("nhh",inf_id),inf_id],] %>% nrow
#   z[grepl("nhh",inf_id)] %>% nrow
#   
#   new.nhh.ids$new.csid %>% class
#   z[which(inf_id%in%new.nhh.ids$csid),`:=`(inf_id=new.nhh.ids[,new.csid])]
# }
# 
# ##Assign all the new caseids
...
# dep.nHH = nrow(prob_samples[type=="non-HH" & caseid%in%case_data$caseid])/total_new_cases
# 
# ##Now get rid of duplicates
# # If caseIDs were repeated, i.e. allocated to more than 1 source, remove the 
# # duplicate sec case.
