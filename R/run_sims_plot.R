...
res.no.tracing2$intervention <- "Isolation (all cases)"

res.no.tracing.gen2 <- res.ext$res.gen
res.no.tracing.gen2$intervention <- "Isolation (all cases)"
...
res <- bind_rows(list(res.no.int,res.no.tracing,res.no.tracing2,res.tracing.HH))
...
figname <- 'plots/CT_all_100sims_20asymp'
...
res %>% 
  subset(week<=cap_weeks) %>%
  # subset(week<cap_weeks) %>% 
  mutate(hh.con=ifelse(intervention%in%c('No Interventions','Isolation only','Isolation only severe','Isolation (all cases)'),0,hh.con)) %>% 
